...
- Windows では **「Windows Firewall でもブロック」** にチェックすると、hosts に加えてファイアウォールでも遮断します（管理者権限が必要）。
...
7. **Windows Firewall でのブロックを試す**  
   hosts では効かない環境向けに、Windows なら **ファイアウォールでもブロック**できます（GUI のチェック or `--use-firewall`）。
...
- `--use-firewall`: Windows Firewall でもブロック（Windows のみ）。
